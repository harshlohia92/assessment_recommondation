...
LOG_FILE = Path("/tmp/recommendation_history.json")
...
def initialize_component():
    ...
    if llm is None:
        if not GEMINI_API_KEY or not GEMINI_MODEL:
            raise ValueError("GEMINI_API_KEY or GEMINI_MODEL not set!")
...
def load_assessments(json_file):
    docs = []
...
"""
    ...
    return {"message": f"💾 Saved recommendation to {LOG_FILE.name}", "recommendation": recommendation, "reasoning": reasoning}
...
if __name__ == "__main__":
    json_file = Path(__file__).parent / ".json"
